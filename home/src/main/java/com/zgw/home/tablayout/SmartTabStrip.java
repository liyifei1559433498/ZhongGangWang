...
class SmartTabStrip extends LinearLayout {
    ...
    private boolean distributeIndication = true;
    /**是否显示底部的driverLine*/
    private boolean isShowDriverLine = true;
    ...
    void setShowDriverLine(boolean isShowDriverLine){
        this.isShowDriverLine = isShowDriverLine;
    }

    void setIndicatorWithoutPadding(boolean withoutPadding) {
        this.indicatorWithoutPadding = withoutPadding;
    }

    void setCustomTabView(int textViewId) {
        this.customTabTextViewId = textViewId;
    }

    void setDistributeEvenly(boolean distributeEvenly) {
        this.distributeEvenly = distributeEvenly;
    }

    void setDistributeIndication(boolean distributeIndication) {
        this.distributeIndication = distributeIndication;
    }
    ...
    private void drawDecoration(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            if (distributeIndication && distributeEvenly
                    && indicationInterpolator instanceof SmartTabIndicationInterpolator.SmartIndicationInterpolator) {
                if (maxTabWidth == 0) {
                    for (int i = 0; i < tabCount; i++) {
                        View tabView = getChildAt(i);
                        TextView tabTitleView = null;
                        if (customTabTextViewId != NO_ID && tabView != null) {
                            tabTitleView = (TextView) tabView.findViewById(customTabTextViewId);
                            if (maxTabWidth < tabTitleView.getMeasuredWidth()) {
                                maxTabWidth = tabTitleView.getMeasuredWidth();
                            }
                        }
                    }
                }
                int selectTabWidth = selectedEnd - selectedStart;
                selectedStart = selectedStart + (selectTabWidth - maxTabWidth) / 2;
                selectedEnd = selectedEnd - (selectTabWidth - maxTabWidth) / 2;
            }

            if (!distributeIndication
                    && indicationInterpolator instanceof SmartTabIndicationInterpolator.SmartIndicationInterpolator) {
                try {
                    int tabTxtWidth = 0;
                    View tabView = getChildAt(selectedPosition);
                    TextView tabTitleView = null;
                    if (customTabTextViewId != NO_ID && tabView != null) {
                        tabTitleView = (TextView) tabView.findViewById(customTabTextViewId);
                        tabTxtWidth = tabTitleView.getMeasuredWidth();
                    }

                    int selectTabWidth = selectedEnd - selectedStart;
                    selectedStart = selectedStart + (selectTabWidth - tabTxtWidth) / 2;
                    selectedEnd = selectedEnd - (selectTabWidth - tabTxtWidth) / 2;
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                if (distributeIndication && distributeEvenly
                        && indicationInterpolator instanceof SmartTabIndicationInterpolator.SmartIndicationInterpolator ) {
                    int nextTabWidth = nextEnd - nextStart;
                    nextStart = nextStart + (nextTabWidth - maxTabWidth) / 2;
                    nextEnd = nextEnd - (nextTabWidth - maxTabWidth) / 2;
                }
                ...
                if (!distributeIndication
                    ...
                    int tabTxtWidth = 0;
                    TextView tabTitleView = null;
                    if (customTabTextViewId != NO_ID && nextTab != null) {
                        tabTitleView = (TextView) nextTab.findViewById(customTabTextViewId);
                        tabTxtWidth = tabTitleView.getMeasuredWidth();
                    }

                    int nextTabWidth = nextEnd - nextStart;
                    nextStart = nextStart + (nextTabWidth - tabTxtWidth) / 2;
                    nextEnd = nextEnd - (nextTabWidth - tabTxtWidth) / 2;
                }
                ...
                if (isLayoutRtl) {
                    ...
                    if (indicationInterpolator instanceof SmartTabIndicationInterpolator.SmartIndicationInterpolator) {
                        if (lastPosition <= selectedPosition) {
                            startOffset = selectionOffset;
                            endOffset = indicationInterpolator.getRightEdge(selectionOffset);
                        } else {
                            startOffset = indicationInterpolator.getLeftEdge(selectionOffset);
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
                               int color) {
        if (indicatorThickness <= 0 || indicatorWidth == 0 || !isShowDriverLine) {
            return;
        }
