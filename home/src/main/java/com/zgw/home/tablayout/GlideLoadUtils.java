...
public class GlideLoadUtils {
    ...
    public void glideLoad(Context context, String url, ImageView imageView, int default_image) {
        if (context != null) {
            GlideApp.with(context).load(url).error(default_image).into(imageView);
        } else {
            LogUtil.outLog(TAG, "Picture loading failed,context is null");
        }
    }
    ...
    public void glideLoad(Activity activity, String url, ImageView imageView, int default_image) {
        if (!activity.isDestroyed()) {
            GlideApp.with(activity).load(url).error(default_image).into(imageView);
        } else {
            LogUtil.outLog(TAG, "Picture loading failed,activity is Destroyed");
        }
    }

    public void glideLoad(Fragment fragment, String url, ImageView imageView, int default_image) {
        if (fragment != null && fragment.getActivity() != null) {
            GlideApp.with(fragment).load(url).error(default_image).into(imageView);
        } else {
            LogUtil.outLog(TAG, "Picture loading failed,fragment is null");
        }
    }
    ...
    public void glideLoad(android.app.Fragment fragment, String url, ImageView imageView, int default_image) {
        if (fragment != null && fragment.getActivity() != null) {
            ...
            LogUtil.outLog(TAG, "Picture loading failed,android.app.Fragment is null");
        }
    }
}
