...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    private ColorStateList tabViewSelectTextColors;
    ...
    private boolean distributeIndication;
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.tabViewSelectTextColors = ColorStateList.valueOf(TAB_VIEW_TEXT_COLOR);
        ...
        this.tabViewTextViewId = customTabTextViewId;
        ...
        tabStrip.setDistributeEvenly(this.distributeEvenly);
        ...
        tabStrip.setCustomTabView(customTabTextViewId);
    ...
    /**
     * 由于布局共用，无法去除布局的stl_indicatorColor属性，
     * 这里不需要底部指示器的单独控制
     * @param isShow
     */
    public void setShowIndicationDriver(boolean isShow){
        tabStrip.setShowDriverLine(isShow);
    }
    ...
    public void setSelectTabTextColor(int color) {
        tabViewSelectTextColors = ColorStateList.valueOf(color);
        resetTabTextColot();
    }

    public void setTabTextSize(float size) {
        tabViewTextSize = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, size, dm);
    }

    public void setIndicatorWithoutPadding(boolean withoutPadding) {
        tabStrip.setIndicatorWithoutPadding(withoutPadding);
    }
    ...
    public void setDistributeIndication(boolean distributeIndication) {
        this.distributeIndication = distributeIndication;
        tabStrip.setDistributeIndication(distributeIndication);
    }
    ...
    private void resetTabTextColot() {
        for (int i = 0, size = tabStrip.getChildCount(); i < size; i++) {
            View tabView = tabStrip.getChildAt(i);
            tabView.setSelected(currentPosition == i);
            TextView tabTitleView = null;
            if (tabViewTextViewId != NO_ID && tabView != null) {
                tabTitleView = (TextView) tabView.findViewById(tabViewTextViewId);
            } else {
                tabTitleView = (TextView) tabStrip.getChildAt(i);
            }

            if (tabTitleView != null) {
                if (currentPosition == i) {
                    tabTitleView.setTextColor(tabViewSelectTextColors);
                } else {
                    tabTitleView.setTextColor(tabViewTextColors);
                }
            }
        }
    }
    ...
    private void populateTabStrip() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if (distributeEvenly) {
                ...
                if (tabCount > 6) {
                    lp.leftMargin = Utils.dp2px(getContext(), 14);
                    lp.rightMargin = Utils.dp2px(getContext(), 14);
                }
                tabView.setLayoutParams(lp);
            } else {
                LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) tabView.getLayoutParams();
                lp.leftMargin = Utils.dp2px(getContext(), 10);
                lp.rightMargin = Utils.dp2px(getContext(), 10);
                tabView.setLayoutParams(lp);
            }
